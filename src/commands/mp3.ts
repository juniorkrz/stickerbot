...
import { VideoAudioInfo } from '../types/Youtube'
import { getUrlByQuery, getYoutubeVideo, isYouTubeUrl, downloadYoutubeVideo } from '../handlers/youtube'
import fs from 'fs'
...
export const command: StickerBotCommand = {
    ...
    run: async (
        ...
        let audio
        let duration
        let videoResult: VideoAudioInfo | undefined
        ...
        videoResult = await getYoutubeVideo(url)
        if (!videoResult) {
        ...
        duration = audio ? parseInt(audio.approxDurationMs!) : 0
