...
import { removeVip } from '../handlers/db'
...
export const command: StickerBotCommand = {
  name: commandName,
  aliases: ['unvip'],
  desc: 'Remove o usuário mencionado/autor da mensagem citada/número informado dos VIPs do bot.',
  example: undefined,
  needsPrefix: true,
  inMaintenance: false,
  runInPrivate: true,
  ...
  run: async (
    ...
    if (vips?.length == 0) {
      return await sendMessage(
        {
          text: spintax(
            '⚠ {Ei|Opa|Eita|Ops}, o número a ser removido dos VIPs não foi encontrado, ' +
