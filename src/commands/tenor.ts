...
import { bot, tenorSearch } from '../config'
import { makeSticker } from '../handlers/sticker'
import { getTenors } from '../handlers/tenor'
...
export const command: StickerBotCommand = {
  name: commandName,
  aliases: ['tenor', 'tenors'],
  desc: 'Cria um sticker do tema solicitado via Tenor, cria vários se usar o comando no plural.',
  ...
  run: async (
    ...
    // search term
    const customSearch: TenorSearch = {
      ...tenorSearch,
      q: term
    }

    console.log(tenorSearch.q, customSearch.q)
    const searchResult = await getTenors(customSearch)
      ...
      await sendMessage(
        {
          text: spintax(
            ...
            `no Tenor ${getRandomItemFromArray(emojis.confused)}`
          )
        },
        message
      )
