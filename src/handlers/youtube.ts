...
import fs from 'fs'
import { VideoAudioInfo, VideoSearch } from '../types/Youtube'
...
export async function getUrlByQuery(query: string) {
    try {
        ...
        const firstVideo = searchResults.items[0] as VideoSearch | undefined
...
export const getYoutubeVideo = async (url: string): Promise<VideoAudioInfo | undefined> => {
    try {
